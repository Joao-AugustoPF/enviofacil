...
export default function ShipmentDetailPage() {
  ...
  const shipment = useMemo(
    () => shipments.find((s) => s.order === id),
    [shipments, id],
  );
  const detail = shipmentDetails[id || ""] as any;
  const canAdmin = papel === "ADMIN" || papel === "DONO";

  const [manualTracking, setManualTracking] = useState("");

  const hasAutoIntegration = Boolean(
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <div className="text-sm text-muted-foreground">
            Status: <Badge>{shipment.status}</Badge>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          {canAdmin ? (
            <ReemitirDialog
              onConfirm={() => toast.success("Etiqueta reemitida")}
            />
          ) : (
            <Button disabled title="Permissão necessária">
              Reemitir etiqueta
            </Button>
          )}
          {canAdmin ? (
            <CancelarDialog
              onConfirm={(motivo) => toast.success("Envio cancelado")}
            />
          ) : (
            <Button disabled title="Permissão necessária">
              Cancelar envio
          ...
          <Button variant="secondary" asChild>
            <Link
              to={`/shipments/${encodeURIComponent(shipment.order)}/editar`}
            >
              Editar
            </Link>
          </Button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <div className="font-medium">
                {detail?.weightKg} kg •{" "}
                {detail
                  ? `${detail.dimensionsCm.w}x${detail.dimensionsCm.h}x${detail.dimensionsCm.l} cm`
                  : "—"}
              </div>
            </div>
            <div>
              ...
              <div className="font-medium">
                {shipment.value} • {shipment.status}
              </div>
            </div>
      ...
      <Tabs defaultValue="rastreamento">
        ...
        <TabsContent value="rastreamento">
          {/* Campo manual quando não houver integração automática */}
          {!hasAutoIntegration && (
            <Card className="mb-4">
              <CardHeader>
                <CardTitle>Adicionar código de rastreio</CardTitle>
                <CardDescription>
                  Informe o código para acompanhar os eventos.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-3">
                  <Input
                    placeholder="Ex: PY123456789BR"
          ...
          {/* Timeline vertical */}
          <ol className="mt-3 space-y-4 relative">
            {detail?.trackingEvents?.map((ev: any, i: number) => (
              <li key={i} className="flex gap-3 items-start">
                <div className="flex flex-col items-center">
                  <div className="mt-1 h-2 w-2 rounded-full bg-primary" />
                  {i < (detail?.trackingEvents?.length || 0) - 1 && (
          ...
          <div className="mt-4">
            {hasAutoIntegration ? (
              canAdmin ? (
                <Button
                  onClick={() => toast.success("Rastreamento atualizado")}
                >
                  {" "}
                  <RefreshCw className="w-4 h-4 mr-2" /> Atualizar rastreamento
                </Button>
              ) : (
                <Button disabled title="Permissão necessária">
        ...
        <TabsContent value="financeiro">
          <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
            ...
            <div className="rounded-md border p-3">
              ...
              <div className="font-semibold">{shipment.cost}</div>
            </div>
            <div className="rounded-md border p-3">
              <div className="text-muted-foreground">Margem</div>
              <div className="font-semibold">{shipment.margin}</div>
            </div>
            <div className="rounded-md border p-3">
              <div className="text-muted-foreground">Divergências</div>
              <div className="text-xs">Sem divergências</div>
            </div>
          </div>
        </TabsContent>
      </Tabs>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Tem certeza que deseja cancelar este envio? Esta ação não pode ser
            desfeita.
          </DialogDescription>
        </DialogHeader>
        <div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            A etiqueta atual será invalidada.
          </DialogDescription>
        </DialogHeader>
        <div className="text-sm text-muted-foreground">
          Custo e valor serão recalculados no backend.
        </div>
        <DialogFooter>
          <Button
            onClick={() => {
